...
def run_training(args, train_data):
    ...
    if args.resume:
        if "EleutherAI" in args.arch:
            model = transformers.GPTNeoForCausalLM.from_pretrained(args.resume)
        else:
...
def get_dataset(args): 
    ...
    train_data = APPSBaseDataset(
        ...
        problem_dirs=fnames,
        mode=args.arch, 
        max_tokens=2048 if ('EleutherAI' in args.arch or (args.load != None and '2700' in args.load)) else 1024,
...
if __name__ == "__main__":
    ...
    # Dataloading
    parser.add_argument('--apps-dataroot', default='./CONALA/train', type=str)
    parser.add_argument('--apps-train-files', default='./CONALA/train', type=str)
